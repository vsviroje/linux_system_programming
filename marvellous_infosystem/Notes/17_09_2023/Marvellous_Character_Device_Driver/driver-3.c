...
#define  DEVICE_NAME "VSV_Driver_1"

// The device class -- this is a character device driver
#define  CLASS_NAME  "VSV_Driver"        

// The license type -- this affects available functionality
MODULE_LICENSE("GPL");

// The author -- visible when you use modinfo
MODULE_AUTHOR("Vardhaman Viroje");


MODULE_DESCRIPTION("vsv : Demo character driver");
...
{
	printk(KERN_INFO "vsv : Driver loaded successfully\n");
	...
	if (majorNumber<0)
	{
		printk(KERN_ALERT "vsv : failed to register a major number\n");

		return majorNumber;
	}

	printk(KERN_INFO "vsv : registered correctly with major number %d\n", majorNumber);
	...
	printk(KERN_INFO "vsv : device class registered correctly\n");
	...
	printk(KERN_INFO "vsv : device class created correctly\n");
...
static void __exit char_exit(void)
{
	...
	unregister_chrdev(majorNumber, DEVICE_NAME);
	printk(KERN_INFO "vsv : Goodbye from our driver!\n");
}
...
static int dev_open(struct inode *inodep, struct file *filep)
{
	...
	printk(KERN_INFO "vsv :  Device has been opened %d time(s)\n", numberOpens);
...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset)
{
	...
	{            // if true then have success
		printk(KERN_INFO "vsv :  Sent %d characters to the user\n", size_of_message);
		
		return (size_of_message=0);  // clear the position to the start and return 0
	}
	else 
	{
		printk(KERN_INFO "vsv :  Failed to send %d characters to the user\n", error_count);
...
{
	printk(KERN_INFO "vsv : write operation start\n", len);

	copy_from_user(message, buffer, len);

	size_of_message = len;                 // store the length of the stored message

	printk(KERN_INFO "vsv :  Received %d characters from the user\n", len);
...
{
	printk(KERN_INFO "vsv :  Device successfully closed\n");
